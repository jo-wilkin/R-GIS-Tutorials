...
```
[Lecture slides](LINK) [Watch on MS stream](MS-STREAM LINK)
<br>
...
#### Video: What are the Sustainable Development Goals and the Data for Good sector?    
<br>

The first lecture this week provides an introduction to the Sustainable Development Goals (SDGs) and the Data for Good sector. It outlines the 17 goals and their targets, as well as why data is essential for their achievement and success.
...
#### Data For Good initiatives and resulting datasets
<br>
...
library(vembedr)
embed_msstream('token') %>% use_align('left')
```
[Lecture slides](LINK) [Watch on MS stream](MS-STREAM LINK)
<br><br>
...
library(mapview)
# Create Tanzania map
tanzania_borders <- st_read('raw/w8/raw/administrative_boundaries/gadm36_TZA_0.shp')
```
<center>
...
Whilst you should have already come across DB-Scan in your CASA0005 module, in today's practical you'll see how it can be applied within novel urban mapping applications and data science. Here we provide you with a quick refresher to how DB-Scan works: 

```{r 08-short-lecture-DB-Scan, warnings=FALSE, message=FALSE, echo=FALSE}
library(vembedr)
embed_msstream('') %>% use_align('left')
```
[Lecture slides](LINK) [Watch on MS stream](MS-STREAM LINK)<br><br>
...
```{r 08-reading-data, warnings=FALSE, message=FALSE, cache=TRUE}
...
tnz_bf <- st_read('raw/w8/raw/building_footprints/Tanzania.geojson')

# If you have the subset, you'll need to load the subset in instead:
#arusha_bf <- st_read('raw/w8/arusha_building_footprints.shp')

# read our district level adminstrative data into spatial dataframe
tnz_districts <- st_read('raw/w8/raw/administrative_boundaries/gadm36_TZA_2.shp')
```
...
```{r 08-subsetting-building-footprints, warnings=FALSE, message=FALSE, cache=TRUE}
...
st_write(settlement_hulls, "raw/w8/settlement_outlines.shp")
